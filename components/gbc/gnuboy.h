...
// ★ ВМЕСТО rg_system.h ИСПОЛЬЗУЕМ НАШИ МАКРОСЫ ★
#include "rg_system.h"

// ============================================================================
//  ОПРЕДЕЛЕНИЯ КОНСТАНТ (ТОЛЬКО ЗДЕСЬ!)
// ============================================================================
...
#define LOG_PRINTF(level, x...) rg_system_log(level, NULL, x)

#define MESSAGE_ERROR(x, ...) LOG_PRINTF(RG_LOG_ERROR, "!! %s: " x, __func__, ## __VA_ARGS__)
#define MESSAGE_WARN(x, ...)  LOG_PRINTF(RG_LOG_WARN,  "** %s: " x, __func__, ## __VA_ARGS__)
#define MESSAGE_INFO(x, ...)  LOG_PRINTF(RG_LOG_INFO,  " * %s: " x, __func__, ## __VA_ARGS__)
#define MESSAGE_DEBUG(x, ...) // LOG_PRINTF(RG_LOG_DEBUG, ">> %s: " x, __func__, ## __VA_ARGS__)
