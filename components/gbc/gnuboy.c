...
void gnuboy_run(bool draw)
{
    ...
    run_count++;
    if (run_count % 100 == 0) {
        MESSAGE_INFO("gnuboy_run: frame %d, draw=%d, blit_func=%p\n", 
                     run_count, draw, host.video.blit_func);
    }
    ...
    if (draw && host.video.blit_func) {
        ...
        if (warn_count++ % 100 == 0) {
            MESSAGE_WARN("blit not called: draw=%d, func=%p\n", draw, host.video.blit_func);
        }
    }
...
void gnuboy_load_bank(int bank)
{
    ...
    if (!cart.rombanks[bank]) {
        ...
        if (!cart.rombanks[bank]) {
            MESSAGE_ERROR("Failed to allocate bank %d\n", bank);
            return;
        }
    }

    if (!cart.romFile) {
        MESSAGE_ERROR("cart.romFile is NULL!\n");
        return;
    }
    ...
    if (fseek(cart.romFile, OFFSET, SEEK_SET) != 0 || 
        ...
        MESSAGE_WARN("ROM bank loading failed for bank %d\n", bank);
...
int gnuboy_load_rom(const char *file)
{
    ...
    // ★ ПРЕОБРАЗОВАНИЕ ПУТИ ★
    char real_path[256];
    if (strncmp(file, "A:/", 3) == 0) {
        snprintf(real_path, sizeof(real_path), "/sdcard%s", file + 2);
    } else {
        strncpy(real_path, file, sizeof(real_path) - 1);
        real_path[sizeof(real_path) - 1] = '\0';
    }

    MESSAGE_INFO("Trying to open: '%s'\n", real_path);

    cart.romFile = fopen(real_path, "rb");
    if (cart.romFile == NULL) {
        MESSAGE_ERROR("ROM fopen failed: %s (errno=%d)\n", real_path, errno);
        return -1;
    }
